...
export class CoversScene {
  ...
  _spawnSprites() {
    this.scene.clear();
    this.sprites = [];
    const mat = new THREE.SpriteMaterial({ map: this.album, transparent: true, opacity: 0.85, depthWrite: false });
    for (let i=0;i<20;i++){
      const s = new THREE.Sprite(mat.clone());
      s.position.set((Math.random()-0.5)*14, (Math.random()-0.5)*8, (Math.random()-0.5)*10);
      const sc = 0.8 + Math.random()*1.6;
      s.scale.set(sc, sc, 1);
      s.userData.v = new THREE.Vector3((Math.random()-0.5)*0.02, (Math.random()-0.5)*0.02, (Math.random()-0.5)*0.02);
      this.scene.add(s);
      this.sprites.push(s);
    }
  }

  setAlbumTexture(tex) { this.album = tex; this._spawnSprites(); }
  setPalette() {}
  setAnalysis() {}
  setTempo() {}
  onBeat() {
